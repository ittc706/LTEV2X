...
RRM_TDM_DRA_VeUE::RRM_TDM_DRA_VeUE() :RRM_VeUE(RRM_TDM_DRA::gc_TotalPatternNum) {}
...
void RRM_TDM_DRA_RSU::initialize() {
	...
	m_PatternIsAvailable = vector<vector<bool>>(getSystemPoint()->getGTTPoint()->m_ClusterNum, vector<bool>(RRM_TDM_DRA::gc_TotalPatternNum, true));
	m_ScheduleInfoTable = vector<vector<ScheduleInfo*>>(getSystemPoint()->getGTTPoint()->m_ClusterNum, vector<ScheduleInfo*>(RRM_TDM_DRA::gc_TotalPatternNum, nullptr));
	m_TransimitScheduleInfoList = vector<vector<list<ScheduleInfo*>>>(getSystemPoint()->getGTTPoint()->m_ClusterNum, vector<list<ScheduleInfo*>>(RRM_TDM_DRA::gc_TotalPatternNum));
}


int RRM_TDM_DRA_RSU::getClusterIdx(int t_TTI) {
	int roundATTI = t_TTI%RRM_TDM_DRA::gc_NTTI; //将TTI映射到[0-gc_DRA_NTTI)的范围
...
const int RRM_TDM_DRA::gc_RBNumPerPatternType[gc_PatternTypeNum] = { 2,10,10 };

const int RRM_TDM_DRA::gc_PatternNumPerPatternType[gc_PatternTypeNum] = { 0,5,0 };

const int RRM_TDM_DRA::gc_PatternTypePatternIdxInterval[gc_PatternTypeNum][2] = {
...
const int RRM_TDM_DRA::gc_TotalPatternNum = [&]() {
	int res = 0;
...
RRM_TDM_DRA::RRM_TDM_DRA(System* t_Context) :
	...
	m_InterferenceVec = vector<vector<list<int>>>(getContext()->m_Config.VeUENum, vector<list<int>>(gc_TotalPatternNum));
...
void RRM_TDM_DRA::cleanWhenLocationUpdate() {
	for (int VeUEId = 0; VeUEId < getContext()->m_Config.VeUENum; VeUEId++) {
		...
		m_VeUEAry[VeUEId]->m_PreSINR.assign(gc_TotalPatternNum, (numeric_limits<double>::min)());
	}
}
...
void RRM_TDM_DRA::groupSizeBasedTDM(bool t_ClusterFlag) {
	...
	for (int RSUId = 0; RSUId < getContext()->m_Config.RSUNum; RSUId++) {
		...
		if (_RSU->getSystemPoint()->getGTTPoint()->m_VeUEIdList.size() == 0) {
			...
			*------------------------ATTENTION-----------------------*/
			_RSU->getTDM_DRAPoint()->m_ClusterTDRInfo = vector<tuple<int, int, int>>(_RSU->getSystemPoint()->getGTTPoint()->m_ClusterNum, tuple<int, int, int>(0, gc_NTTI - 1, gc_NTTI));
			continue;
		}
		...
		double VeUESizePerTTI = static_cast<double>(_RSU->getSystemPoint()->getGTTPoint()->m_VeUEIdList.size()) / static_cast<double>(gc_NTTI);
		...
		int remainNTTI = gc_NTTI - basicNTTI;
...
void RRM_TDM_DRA::uniformTDM(bool t_ClusterFlag) {
	...
	for (int RSUId = 0; RSUId < getContext()->m_Config.RSUNum; RSUId++) {
		...
		int equalTimeLength = gc_NTTI / _RSU->getSystemPoint()->getGTTPoint()->m_ClusterNum;

		int lastClusterLength = gc_NTTI - equalTimeLength*_RSU->getSystemPoint()->getGTTPoint()->m_ClusterNum + equalTimeLength;
		...
		get<1>(_RSU->getTDM_DRAPoint()->m_ClusterTDRInfo[_RSU->getSystemPoint()->getGTTPoint()->m_ClusterNum - 1]) = gc_NTTI - 1;
...
void RRM_TDM_DRA::processScheduleInfoTableWhenLocationUpdate() {
	for (int RSUId = 0; RSUId < getContext()->m_Config.RSUNum; RSUId++) {
		...
		for (int clusterIdx = 0; clusterIdx < _RSU->getSystemPoint()->getGTTPoint()->m_ClusterNum; clusterIdx++) {
			for (int patternIdx = 0; patternIdx < gc_TotalPatternNum; patternIdx++) {
...
void RRM_TDM_DRA::selectRBBasedOnP123() {
	for (int RSUId = 0; RSUId < getContext()->m_Config.RSUNum; RSUId++) {
		...
		for (int clusterIdx = 0; clusterIdx < _RSU->getSystemPoint()->getGTTPoint()->m_ClusterNum; clusterIdx++) {
			...
			for (int patternIdx = 0; patternIdx < gc_PatternNumPerPatternType[EMERGENCY]; patternIdx++) {
		...
		*/
		vector<vector<int>> curAvaliablePatternIdx(gc_PatternTypeNum);

		for (int patternTypeIdx = 1; patternTypeIdx < gc_PatternTypeNum; patternTypeIdx++) {
			for (int patternIdx = gc_PatternTypePatternIdxInterval[patternTypeIdx][0]; patternIdx <= gc_PatternTypePatternIdxInterval[patternTypeIdx][1]; patternIdx++) {
				if (_RSU->getTDM_DRAPoint()->m_PatternIsAvailable[clusterIdx][patternIdx]) {
...
void RRM_TDM_DRA::delaystatistics() {
	for (int RSUId = 0; RSUId < getContext()->m_Config.RSUNum; RSUId++) {
		...
		//处理此刻正在将要传输的传输链表
		int clusterIdx = _RSU->getTDM_DRAPoint()->getClusterIdx(getContext()->m_TTI);
		for (int patternIdx = gc_PatternNumPerPatternType[EMERGENCY]; patternIdx < gc_TotalPatternNum; patternIdx++) {
			for (RRM_RSU::ScheduleInfo* &p : _RSU->getTDM_DRAPoint()->m_TransimitScheduleInfoList[clusterIdx][patternIdx])
				getContext()->m_EventVec[p->eventId].increaseSendDelay();
		}
	...
	for (int RSUId = t_FromRSUId; RSUId <= t_ToRSUId; RSUId++) {
		...
		for (int clusterIdx = 0; clusterIdx < _RSU->getSystemPoint()->getGTTPoint()->m_ClusterNum; clusterIdx++) {
			for (int patternIdx = 0; patternIdx < gc_PatternNumPerPatternType[EMERGENCY]; patternIdx++) {
				...
				if (lst.size() == 1) {
					...
					int maxEquivalentBitNum = (int)((double)(gc_RBNumPerPatternType[EMERGENCY] * gc_BitNumPerRB)* factor);
		...
		for (int patternIdx = gc_PatternNumPerPatternType[EMERGENCY]; patternIdx < gc_TotalPatternNum; patternIdx++) {
			...
			if (lst.size() == 1) {//只有一个用户在传输，该用户会正确的传输所有数据（在离开簇之前）
				...
				//该编码方式下，该Pattern在一个TTI最多可传输的有效信息bit数量
				int maxEquivalentBitNum = (int)((double)(gc_RBNumPerPatternType[patternType] * gc_BitNumPerRB)* factor);
...
void RRM_TDM_DRA::writeScheduleInfo(ofstream& t_File) {
	...
	for (int RSUId = 0; RSUId < getContext()->m_Config.RSUNum; RSUId++) {
		...
		for (int clusterIdx = 0; clusterIdx < _RSU->getSystemPoint()->getGTTPoint()->m_ClusterNum; clusterIdx++) {
			...
			for (int patternIdx = gc_PatternTypePatternIdxInterval[EMERGENCY][0]; patternIdx <= gc_PatternTypePatternIdxInterval[EMERGENCY][1]; patternIdx++) {
			...
			t_File << "            PERIOD:" << endl;
			for (int patternIdx = gc_PatternTypePatternIdxInterval[PERIOD][0]; patternIdx <= gc_PatternTypePatternIdxInterval[PERIOD][1]; patternIdx++) {
				bool isAvaliable = _RSU->getTDM_DRAPoint()->m_PatternIsAvailable[clusterIdx][patternIdx];
				t_File << "                Pattern[" << left << setw(3) << patternIdx << "] : " << (isAvaliable ? "Available" : "Unavailable") << endl;
				if (!isAvaliable) {
					RRM_RSU::ScheduleInfo *info = *(_RSU->getTDM_DRAPoint()->m_TransimitScheduleInfoList[clusterIdx][patternIdx].begin());
			...
			t_File << "            DATA:" << endl;
			for (int patternIdx = gc_PatternTypePatternIdxInterval[DATA][0]; patternIdx <= gc_PatternTypePatternIdxInterval[DATA][1]; patternIdx++) {
				bool isAvaliable = _RSU->getTDM_DRAPoint()->m_PatternIsAvailable[clusterIdx][patternIdx];
				t_File << "                Pattern[" << left << setw(3) << patternIdx << "] : " << (isAvaliable ? "Available" : "Unavailable") << endl;
				if (!isAvaliable) {
					RRM_RSU::ScheduleInfo *info = *(_RSU->getTDM_DRAPoint()->m_TransimitScheduleInfoList[clusterIdx][patternIdx].begin());
...
int RRM_TDM_DRA::getPatternType(int t_PatternIdx) {
	...
	for (int patternType = 0; patternType < gc_PatternTypeNum; patternType++) {
		if (t_PatternIdx >= gc_PatternTypePatternIdxInterval[patternType][0] && t_PatternIdx <= gc_PatternTypePatternIdxInterval[patternType][1])
			return patternType;
	}
...
pair<int, int> RRM_TDM_DRA::getOccupiedSubCarrierRange(MessageType t_MessageType, int t_PatternIdx) {
	...
	for (int i = 0; i < t_MessageType; i++) {
		offset += gc_RBNumPerPatternType[i] * gc_PatternNumPerPatternType[i];
		t_PatternIdx -= gc_PatternNumPerPatternType[i];
	}
	res.first = offset + gc_RBNumPerPatternType[t_MessageType] * t_PatternIdx;
	res.second = offset + gc_RBNumPerPatternType[t_MessageType] * (t_PatternIdx + 1) - 1;
